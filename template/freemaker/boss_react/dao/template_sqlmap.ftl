...
<mapper namespace="${tb.fLowerTName}">
    ...
    <!-- 修改条件 -->
	<sql id="update${tb.fUpperTName}ByCondition">
        <where>
    ...
    <update id="logicDelete${tb.fUpperTName}ByBatchId" parameterType="map" >
    	...
    	${item.keyStr} in <foreach item="item" index="index" collection="batchId${x}" open="(" separator="," close=")">#${"{item}"} <#assign x=x+1 /></foreach><#if item_has_next> and </#if>
    	</#list>
    </update>
...
<#list tb.columns as item>
	...
	<#list tb.columns as item>
		...
		<#if (tb.tableKey?size>0) > 
		<include refid="update${tb.fUpperTName}ByCondition" />
		</#if>
    </update>
    <!-- 根据主键进行批量修改 -->
	...
	<select id="get${tb.fUpperTName}ById" parameterType="${tb.fLowerTName}Query" resultMap="${tb.fLowerTName}Result">
		SELECT <include refid="${tb.fLowerTName}Field" /> from  ${tb.tableName} <#if (tb.tableKey?size>0) > where <#list tb.tableKey as item>${item.keyStr} = #${"{${item.fLowerkey}}"} and ifnull(del_state,0) = 0 <#if item_has_next> and </#if></#list> </#if>
	</select>
