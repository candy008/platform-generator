...
<#list tb.columns as item>
	...
	<select id="get${tb.fUpperTName}ByPage" parameterType="${tb.fLowerTName}Query" resultMap="${tb.fLowerTName}Result">
		SELECT <include refid="${tb.fLowerTName}Field" /> from
		(
			SELECT A.*, ROWNUM RN 
			FROM 
			(
	...
	<select id="get${tb.fUpperTName}ById" parameterType="${tb.fLowerTName}Query" resultMap="${tb.fLowerTName}Result">
		SELECT <include refid="${tb.fLowerTName}Field" /> from  ${tb.tableName} <#if (tb.tableKey?size>0) > where <#list tb.tableKey as item>${item.keyStr} = #${"{${item.fLowerkey}}"}<#if item_has_next> and </#if></#list> </#if>
	</select>
