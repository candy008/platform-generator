...
<div class="${tb.fLowerTName}DataGridTb" style="height:auto" >
	...
	<a href="javascript:void(0)"  class="easyui-linkbutton save-btn type-disable" data-options="iconCls:'icon-edit',plain:true" >保存</a>
	<a href="javascript:void(0)"  class="easyui-linkbutton cancel-btn type-disable" data-options="iconCls:'icon-remove',plain:true" >取消</a>
	<!-- 工具条 end -->
	<!-- 查询form表单 start -->
	...
	jQuery(function(){ 
		...
		jQuery('#${tb.fLowerTName}Module .${tb.fLowerTName}DataGridTb .delete-btn').linkbutton({
			onClick:function(){
				...
				jQuery('#${tb.fLowerTName}Module .${tb.fLowerTName}DataGrid').datagrid("deleteRow",{
					searchFormId:"search${tb.fUpperTName}FormId",
					url:url,
