...
<div id="${tb.fLowerTName}Module" class="easyui-panel" data-options="fit:true,border:false">
	...
	<div class="easyui-layout" data-options="fit:true" >
	    ...
	    <div data-options="region:'west',split:true" style="width:200px;border-right-width:1px;" border="false" >
			<ul class="easyui-tree ${tb.fLowerTName}Tree"></ul>
	    </div>
	    <!-- layout left -->
...
<script>
	...
	jQuery('#${tb.fLowerTName}Module .${tb.fLowerTName}DataGridTb .add-btn').linkbutton({
		onClick:function(){
			...
			var selectData = jQuery('#${tb.fLowerTName}Module .${tb.fLowerTName}Tree').tree('getSelected');
			if(selectData==null){
				jQuery.messager.alert('错误提示','请选择资源分类树节点！','info');
				return;		
			}
			jQuery('#${tb.fLowerTName}EditWin').window('open');
			jQuery("#${tb.fLowerTName}EditWin .${tb.fLowerTName}EditForm input[textboxname='pid']").textbox('setValue',selectData.id);
		}
	})
	...
	/**
     * ${tb.fLowerTName}的tree
     */
	jQuery('#${tb.fLowerTName}Module .${tb.fLowerTName}Tree').tree({
		url:'$homeModule.getTarget("/tree_data1.json")',
		method:'get',
		animate:true,
		onClick: function(node){
			var id = node.id;
		    jQuery("#${tb.fLowerTName}Module .search${tb.fUpperTName}FormId input[name='id']").val(id);
			gridRefreshTree('#${tb.fLowerTName}Module .${tb.fLowerTName}DataGrid','#${tb.fLowerTName}Module .search${tb.fUpperTName}FormId','$homeModule.getTarget("/${tb.fLowerTName}/get${tb.fUpperTName}ByPage")');
		}
	});
